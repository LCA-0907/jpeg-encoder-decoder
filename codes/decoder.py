...
def read_image_file(filepath, block_size):
    ...
    for block_index in range(blocks_count):
        for component in range(3):
            ...
            while cells_count < block_size**2-1:
...
def dequantize(block, component, size, quantlv=1):
    q = load_quantization_table(component, size, quantlv)
    return block * q
...
def PSNR(im_origin, im_compress):
    mse = np.mean((im_origin - im_compress) ** 2) 
...
def main():
    ...
    for block_index in range(blocks_count):
        ...
        for c in range(3):
            ...
            dct_matrix = dequantize(quant_matrix, 'lum' if c == 0 else 'chrom', block_size, quant_lv)
    ...
    # calculate psnr
    if args.originimg != 'NONE':
        original = cv2.imread(args.originimg) 
        compressed = cv2.imread(args.output) 
        value = PSNR(original, compressed) 
        print(f"Test for {args.output} img")
