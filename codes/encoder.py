...
def quantize(block, component, size, quantlv=1):
...
def chroma_subsample(cb, cr, scale='4.4.4'):
    print('scale: ', scale)
    if scale =='4.2.2':
        newcb = cb.copy()
        newcr = cr.copy()
        for i in range(1, cb.shape[0], 2):
            for j in range(cb.shape[1]):
                # print (i, j)
                newcb[i, j]=(int(cb[i-1, j]) + int(cb[i, j])) /2
                newcr[i, j]=(int(cr[i-1, j]) + int(cr[i, j])) /2
...
def main():
    ...
    chroma_scale = args.chromascale
    quant_lv = args.quantlv
    
    print(args)
    print(chroma_scale)
    print(block_size**2-1)
    ...
    # chroma subsample
    newcb, newcr = chroma_subsample(npmat[:,:,1], npmat[:,:,2], chroma_scale)
    npmat[:,:,1] = newcb
    npmat[:,:,2] = newcr
        ...
        for j in range(0, cols, block_size):
            ...
            for k in range(3):
                ...
                quant_matrix = quantize(dct_matrix,
                                        'lum' if k == 0 else 'chrom',block_size, quant_lv)
